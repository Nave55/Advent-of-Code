...
fn index_to_value(indices: List(Int), n: Int) -> List(Int) {
...
fn parse_input(path: String) -> List(Data) {
...
fn solution1(lst: List(Data)) -> Int {
  use acc, x <- list.fold(lst, 0)
  case press_buttons(x, list.length(x.buttons), 1) {
    Ok(val) -> acc + val
    _ -> acc
  }
}
      ...
      case compute_presses_from_rat(mat, free_cols, vars) {
        Ok(presses) -> {
          case list.all(presses, fn(x) { x >= 0 }) {
            True -> {
              case verify_solution(buttons, target, presses, False, num) {
                True -> {
...
pub fn solution2(lst: List(Node)) -> Result(Rat, Node) {
  list.try_fold(lst, #(0, 0), fn(acc, n) {
    ...
    let sol = enumerate_feasible_incremental(re, free, n.buttons, n.target, ind)
    let total = list.fold(sol, 0, fn(acc, x) { acc + x })
    let ok = verify_solution(n.buttons, n.target, sol, True, ind)
    case ok {
      True -> Ok(#(ttl + total, ind + 1))
      False -> Error(n)
    }
  })
}
